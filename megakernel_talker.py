...
class TalkerKernel:
    ...
    def prefill_from_cache(self, past_key_values):
        """Copy a PyTorch prefill cache (per layer K,V [1,kv_heads,seq,head_dim])
        into the kernel's KV cache. Returns the prefill length."""
        seq = 0
...
class MegakernelTalkerGraph:
    """Drop-in replacement for faster_qwen3_tts's TalkerGraph: runs the talker
    decode on the megakernel instead of a captured CUDA graph. Same interface
    used by streaming.py / generate.py (prefill_kv, set_generation_state, run).

    Assumes rope_deltas == 0 (true for custom_voice / voice_clone, where
    model.py sets talker.rope_deltas = None) so RoPE position == cache position.
    """
    ...
    def __init__(self, talker_model, talker_config, device="cuda",
        ...
        self.dtype = dtype
        self.max_seq_len = min(max_seq_len, MAX_SEQ_LEN)
        self.num_layers = talker_config.num_hidden_layers
        self.captured = True
        self._tk = TalkerKernel(build_talker_weights(talker_model))

    def capture(self, prefill_len=100, num_warmup=3):
        pass  # no CUDA graph needed for the megakernel
    ...
    def prefill_kv(self, past_key_values):
        self._tk.reset()
        return self._tk.prefill_from_cache(past_key_values)

    def set_generation_state(self, attention_mask, rope_deltas):
        if rope_deltas is not None and float(rope_deltas.abs().max()) != 0.0:
            print("WARNING: megakernel talker assumes rope_deltas=0; nonzero ignored")

    @torch.inference_mode()
    def run(self, input_embeds, position):
        out = self._tk.step(input_embeds.to(self.dtype), position)  # [H] f32
        return out.to(self.dtype).view(1, 1, HIDDEN_SIZE)


def _e2e_check():
    """Swap the megakernel talker into a real generation and synthesize audio."""
    import soundfile as sf
    from faster_qwen3_tts import FasterQwen3TTS

    print("loading + swapping megakernel talker...")
    ...
    text = "Hello, this speech is generated by the megakernel talker on the RTX 5090."
    wavs, sr = m.generate_custom_voice(text=text, language="English", speaker="ryan")
    sf.write("/workspace/megakernel_e2e.wav", wavs[0], sr)
    print(f"wrote /workspace/megakernel_e2e.wav  dur={len(wavs[0])/sr:.2f}s  sr={sr}")
    print("E2E: audio generated with megakernel talker")
...
def _multistep_check():
    """Prefill via PyTorch, copy KV into the kernel, decode the rest on the
    kernel, compare each step to the full PyTorch reference. Tests the prefill
    KV-copy convention + multi-step decode (the integration's core risk)."""
    from faster_qwen3_tts import FasterQwen3TTS
    from transformers import DynamicCache

    print("loading talker (multistep)...")
    m = FasterQwen3TTS.from_pretrained("Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice")
    base = m.talker_graph.model
    tk = TalkerKernel(build_talker_weights(base))

    torch.manual_seed(1)
    N, M = 8, 4  # N total steps, first M are prefill
    hs = [torch.randn(HIDDEN_SIZE, dtype=torch.bfloat16, device="cuda") for _ in range(N)]

    # Full PyTorch reference
    ref, cache = [], DynamicCache()
    with torch.no_grad():
        for i, h in enumerate(hs):
            o = base(inputs_embeds=h.view(1, 1, HIDDEN_SIZE), use_cache=True,
                     past_key_values=cache, cache_position=torch.tensor([i], device="cuda"))
            ref.append(o.last_hidden_state[0, 0].to(torch.bfloat16))

    # PyTorch prefill first M, copy to kernel, decode M..N-1 on the kernel
    pre = DynamicCache()
    with torch.no_grad():
        for i in range(M):
            base(inputs_embeds=hs[i].view(1, 1, HIDDEN_SIZE), use_cache=True,
                 past_key_values=pre, cache_position=torch.tensor([i], device="cuda"))
    tk.reset()
    tk.prefill_from_cache(pre)

    ok = True
    for i in range(M, N):
        out = tk.step(hs[i], position=i).to(torch.bfloat16)
        cos = torch.nn.functional.cosine_similarity(out.float(), ref[i].float(), dim=0).item()
        print(f"  step {i}: cosine_sim={cos:.5f}")
        ok = ok and cos > 0.99
    print("MULTISTEP+PREFILL:", "PASS" if ok else "FAIL (investigate KV convention)")
